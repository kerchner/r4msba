...
You can write data frames to CSV files using the `write.csv()` function. For example, to write a data frame named `df` to a file named `output.csv`, you can do:

```r
write.csv(df, 'data/output.csv', row.names = FALSE)
```

The `row.names = FALSE` argument prevents R from writing row names (which are often just the row numbers) to the CSV file.

If you wish to write data to an Excel file, you can use the `writexl` package.
